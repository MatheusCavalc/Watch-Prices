  ...
  <div>
    ...
    <ul>
      <li v-for="(product, index) in exactMatches" :key="index">
        <p v-if="product.title && product.price">{{ product.title }} - <span v-if="product.price">{{ product.price }}</span> <span v-else>Indisponivel</span>
          - {{ product.site }}  {{ index }}</p>
      </li>
    </ul>
